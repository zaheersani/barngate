...
function excerpttoRSS() {
    global $post;
    return get_the_excerpt($post->ID);

}
add_filter( 'the_excerpt_rss', 'excerpttoRSS' );
...
function featuredtoRSS() {
    ...
    if ( has_post_thumbnail( $post->ID ) ){
        echo "\t" . '<image>' . get_the_post_thumbnail_url( $post->ID,  array(300, 300) )  . '</image>' . "\n";
    }

}

add_filter('rss2_item', 'featuredtoRSS');
